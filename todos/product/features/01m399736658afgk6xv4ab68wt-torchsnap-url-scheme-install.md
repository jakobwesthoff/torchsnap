...
Parked until there is a distribution story (docs install buttons, a
gadget directory). Needs an ADR before implementation. The design
decisions below were made on 2026-09-24 and go into that ADR.
...
- **Direct URL:** `torchsnap://install?url=https%3A%2F%2F…/foo.torchsnap`.
  Works today with GitHub release assets and needs no registry. A
  registry-based `torchsnap://install/<gadget-id>` was considered and
  not chosen.
- **Ask, then download.** A first prompt names the URL and host
  ("Download gadget from github.com?"). Only after that does
  Torchsnap download, then the normal review follows. No web page can
  make Torchsnap fetch anything without a click.
- **Any HTTPS host**, with the final host after redirects shown
  prominently. No allowlist.
...
## Trust and security points for the ADR
...
- **Download rules.**
  ...
  - The 16 MiB staging cap, enforced while streaming.
