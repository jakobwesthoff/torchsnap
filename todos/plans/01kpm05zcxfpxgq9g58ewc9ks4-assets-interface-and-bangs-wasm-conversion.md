...
- **PR 1** introduces the `assets` host WIT interface so WASM plugins can read files bundled inside their own `.torchsnap` archive (or development directory). No permission declaration — plugins can always read their own assets, security is guaranteed by `validate_plugin_path` rejecting anything that escapes the plugin root. Mirrors the opener/http wiring landed in commit `a20c223`.
- **PR 2** ports the native `bangs` plugin to WASM, using the new `assets` interface (to load the bundled `bang.json` at enable-time) plus the already-landed `http` and `opener` interfaces. Deletes the native plugin, the native frontend settings component, and the `src-tauri/derived/bang.json` include.
...
### Step 1a — Bridge owns its `PluginSource`

**File:** `src-tauri/src/wasm/bridge.rs`

The bridge currently takes `&dyn PluginSource` in `new()` (line 133), uses it locally for `read_wasm()` + migration-file reads, and drops it. `assets` needs the source handle to survive past construction so `enable()` can stash it on `PluginState`. This is a prerequisite — the rest of PR 1 won't compile without it.

Changes:

1. Add a field to `WasmPluginBridge` (next to `sql_config` at line 56):
   ```rust
   /// Retained so `enable()` can stash it on `PluginState` for the
...
2. Change the `new()` signature (line 129) to accept `source: Arc<dyn PluginSource + Send + Sync>` instead of `source: &dyn PluginSource`. The trait is already `Send + Sync` (see `source.rs:77`).
...
After this step: `cargo test -p torchsnap-tauri wasm::bridge` — every existing test still passes, no behavior change yet. Commit: `wasm/bridge: retain PluginSource as Arc for capability imports`.

---

### Step 1b — WIT: add the `assets` interface

**File:** `plugins/plugin-sdk/wit/torchsnap-plugin.wit`
...
```wit
/// Per-plugin asset access.
///
/// Every WASM plugin can read files from inside its own `.torchsnap`
/// archive (or development directory) via this interface. Plugins do
/// NOT opt in — there is no `[permissions.assets]` section. The
/// guarantee is spatial: paths are validated by the same
/// `validate_plugin_path` guard that governs every other plugin-file
...
/// host holds no cached handle. Plugins that need to retain an asset
...
interface assets {
  ...
  variant assets-error {
    /// The path is outside the plugin root, or syntactically invalid
    /// (absolute, backslash-separated, contains `..` traversal, NUL
    /// byte, Windows drive letter, or empty). Message includes the
    /// offending path.
    invalid-path(string),
    /// The path validates but no such file exists inside the plugin.
    not-found,
  ...
  /// Read a file inside the plugin. `path` is relative to the plugin
  /// root, same convention as `manifest.toml` path fields.
  read: func(path: string) -> result<list<u8>, assets-error>;

  /// Check whether a file exists inside the plugin without reading
...
### Step 1c — `PluginSource::file_exists` trait method

**File:** `src-tauri/src/wasm/source.rs`

Add a new trait method on `PluginSource` (around line 85, after `read_file`):

```rust
/// Check whether a file exists at `path` without reading its
/// bytes. Used by the `assets::exists` host import so plugins
...
- `file_exists_returns_true_for_existing_directory_file` (uses `make_plugin_dir`)
...
After this step: `cargo test -p torchsnap-tauri wasm::source`. Commit: `wasm/source: add PluginSource::file_exists for cheap asset probes`.

---

### Step 1d — Runtime: `PluginState` field, setters, and `assets::Host` impl

**File:** `src-tauri/src/wasm/runtime.rs`

Add the field to `PluginState` (insert after `http_client` at line 131, following the field comment style used for `clipboard_writer` / `http_client`):

```rust
/// The plugin's own source handle, stashed by the bridge on
/// `enable()` so the `assets::read` / `assets::exists` host
/// imports can read the plugin's bundled files without
...
Initialize to `None` in `WasmRuntime::instantiate` (at `PluginState { ... }` construction around line 900, next to the other capability `None`s).

Also add it to `#[cfg(test)] fn default_for_test` at line 1307.

Add setter/clearer on `WasmPluginInstance` (next to `set_http_client` at line 1136):

```rust
/// Stash the plugin's `PluginSource` handle. Called by the bridge
/// at `enable()`. The `assets::*` host imports use this to read
/// the plugin's bundled files on demand.
...
```rust
/// Classify a `PluginSource` error into the WIT `assets-error`
/// variant. The host-side trait returns `anyhow::Result<...>`;
/// the two known origins of an error are (a) `validate_plugin_path`
...
/// to `IoError`.
///
/// The simpler alternative — calling `validate_plugin_path` in the
...
// plugin's own `PluginSource` (stashed by the bridge on `enable`).
...
- `assets_read_rejects_traversal_path` — construct a `PluginState` via `default_for_test`, stash a source backed by a `make_plugin_dir`, call `read("../escape")` through the `Host` impl method directly. Assert `AssetsError::InvalidPath(msg)` with `msg.contains("escapes")`.
...
- `assets_read_without_plugin_source_returns_io_error` — `plugin_source: None`, expect `AssetsError::IoError("assets not initialized")`.

**Verify during implementation:** the `bindings::torchsnap::plugin::assets::AssetsError` variant path — wit-bindgen may name it `Assets_Error` or similar. Check by running `cargo doc -p torchsnap-tauri` and looking under `src-tauri/target/doc/.../bindings/`.
...
In `Plugin::enable` (line 522), after `instance.set_http_client(...)` at line 572, add:

```rust
// Assets: stash a clone of the source Arc the bridge already
// holds. No permission allowlist — plugins can always read
// their own bundled files; the guarantee is spatial
// (validate_plugin_path confines reads to the plugin root).
instance.set_plugin_source(Arc::clone(&self.plugin_source));
```

In `Plugin::disable` (line 602), next to `instance.clear_http_client()` at line 620, add:
...
After this step: `cargo build -p torchsnap-tauri`. Commit: `wasm/bridge: wire assets plugin source into enable/disable`.

---

### Step 1f — Test fixture: `assets-plugin`

**New directory:** `src-tauri/tests/fixtures/assets-plugin/`

Structure mirrors `opener-http-plugin/`:

- `Cargo.toml` — copy of `opener-http-plugin/Cargo.toml` with `package.name = "torchsnap-test-assets-plugin"`.
- `manifest.toml` — minimal, no `[permissions]`:
  ```toml
  [plugin]
  id = "assets-plugin"
  name = "Assets Test Plugin"
  description = "Test-only fixture exercising the assets host interface"
  version = "0.0.0"
  wasm = "assets_plugin.wasm"
  icon = "heroicons:beaker"
  ```
- `greeting.txt` — a bundled asset with known content (`Hello from the assets fixture\n`).
- `src/lib.rs` — `wit_bindgen::generate!` pointing at `../../../../plugins/plugin-sdk/wit`, same as `opener-http-plugin/src/lib.rs:20–22`. `MessagingGuest::handle_message` dispatches:
...
After this step: `just build-test-fixtures && cargo test -p torchsnap-tauri -- wasm`. Commit: `wasm/tests: add assets-plugin fixture and integration tests`.

---

### Step 1g — SDK re-export

**File:** `plugins/plugin-sdk/src/lib.rs`

Line 93 currently reads `pub use torchsnap::plugin::{clipboard, frecency, http, opener};`. Add `assets`:
...
No other SDK changes — there's no higher-level `assets` helper module (unlike `sql` and `messaging`, which wrap resource handles / JSON parsing). Plugins call `assets::read(path)` directly.

After this step: `cd plugins && cargo check --workspace`. Commit: `plugin-sdk: re-export assets host import`.
...
Add an `### Assets (WASM plugins)` section before the `### Opener` section at line 772. Style matches the existing HTTP/Opener sections landed in commit `f4ea3c3`. Keep it short — the design is self-evident from the interface:

- Three-sentence overview: plugins can read their own bundled files, no permission needed, paths are plugin-relative.
...
just check-wit
just fmt-check-wit
cd plugins && cargo check --workspace
...
**Location:** `plugins/bangs/` as a new workspace member.

**MPL header reminder:** Every new `.rs`, `.ts`, `.tsx`, `.css`, `.sql` file starts with the 3-line MPL 2.0 header.

### Step 2a — Scaffold the plugin crate and workspace membership

**New files:**

- `plugins/bangs/Cargo.toml` — copied from `plugins/calculator/Cargo.toml` (lines 1–20) with these changes:
  ```toml
  [package]
  name = "bangs-plugin"
  version = "0.1.0"
...
- `plugins/bangs/migrations/001_init.sql` — content extracted verbatim from `src-tauri/src/plugins/bangs/schema.rs:21–36`:
  ```sql
  CREATE TABLE bangs (
...
- `plugins/bangs/assets/bang.json` — the 2.2 MB DDG bang database. Treated as a committed asset in the repo (same way `plugins/calculator/frontend/dist/` is committed). Bootstrap: run `just asset-bang-data` and move the output (after step 2g updates it).

**Edit:** `plugins/Cargo.toml` (line 20) — add `"bangs"` to the `members` array:
```toml
members = [
    "plugin-sdk",
    "hello-world",
...
After this step: `cd plugins && cargo check --workspace` (passes — no `lib.rs` yet so no real compilation happens beyond the manifest check; `bun build` is not yet runnable because there's no frontend). Commit: `plugins/bangs: scaffold crate, manifest, migrations, assets`.

---

### Step 2b — Port the guest Rust code to `plugins/bangs/src/lib.rs`

**New file:** `plugins/bangs/src/lib.rs`.

Structure follows `plugins/calculator/src/lib.rs`:
...
// `thread_local!` block at plugins/calculator/src/lib.rs:75.
thread_local! {
...
- `on_setting_changed`: no-op (the bangs plugin has no settings other than the host-managed `enabled.bangs`).

**`SearchGuest`:**
- `entries()`: `Vec::new()` — bangs is query-only, no catalog.
- `search(query, _prefix)`: direct port of `src-tauri/src/plugins/bangs/mod.rs:160–229`. Differences:
...
**`MessagingGuest`:**
Port `src-tauri/src/plugins/bangs/mod.rs:282–316` dispatch:
...
**`TasksGuest`:** use `impl_noop_tasks!(BangsPlugin);` from the SDK (see `plugins/plugin-sdk/src/lib.rs:166`). No scheduled refresh — matches the native behavior exactly (network-try-only on enable if DB empty).

**Import helpers — port `src-tauri/src/plugins/bangs/import.rs`:**
...
**Query-token helpers** (`find_bang_token`, `byte_offset_of_token`, `remove_bang_token`): verbatim port from `src-tauri/src/plugins/bangs/mod.rs:360–413`. These are pure functions with no host dependencies.
...
After this step: `cd plugins && cargo check --manifest-path bangs/Cargo.toml` and `just build-plugin bangs` (produces `plugins/bangs/bangs_plugin.wasm` and `plugins/bangs.torchsnap`). Commit: `plugins/bangs: port guest to WASM`.

---

### Step 2c — Port the frontend settings component

**New directory:** `plugins/bangs/frontend/`, scaffolded from `plugins/calculator/frontend/`:
...
- `src/BangsSettings.tsx` — moved from `src/plugins/bangs/BangsSettings.tsx` (181 lines) with import rewrites:
  - `import { sendPluginMessage } from "../../lib/pluginMessage";` → remove; replace with the SDK hook pattern used by calculator:
    ...
    ```
    Then inside the component: `const { sendMessage } = usePluginRuntime();` and replace the old `pluginMessage<T>(method, payload)` wrapper with `sendMessage<unknown, T>(method, payload)` calls.
...
Change the `dst` path at line 13 from `src-tauri/derived/bang.json` to `plugins/bangs/assets/bang.json`. The recipe becomes:

```just
# Download DuckDuckGo bang database into plugins/bangs/assets/
asset-bang-data:
    #!/usr/bin/env bash
    set -euo pipefail
    dst="plugins/bangs/assets/bang.json"
    mkdir -p "$(dirname "$dst")"
...
No other changes needed. The `package-plugin` recipe in `just/plugins.just` already blacklists `src/`, `Cargo.toml`, `frontend/src/`, etc. — it does not exclude `assets/`, so the committed `bang.json` ships inside the `.torchsnap` archive automatically. **Verify during implementation:** confirm `plugins/bangs/assets/` isn't accidentally excluded by the blacklist (reading `just/plugins.just:127-141` shows it is not).

After this step: `just asset-bang-data` (writes the 2.2 MB file to the new location). Commit: `just/bangs: move asset download into plugins/bangs/assets/`.

---

### Step 2e — Add `bangs` to the bundled plugins list

**File:** `plugins/bundled.toml`
...
After this step: `just stage-bundled-plugins` produces `target/bundled-plugins/bangs.torchsnap`. Commit: `plugins: add bangs to bundled.toml`.
...
- `src-tauri/src/plugins/bangs/mod.rs` (497 lines)
...
- `src-tauri/src/plugins/mod.rs:23` — remove `pub mod bangs;` line.
- `src-tauri/src/lib.rs:658–663` — remove the `host.register(Box::new(plugins::bangs::BangsPlugin::new(...)), wasm::source::PluginSourceKind::Builtin);` block. `metadata_service` stays — it's still used by `open_url` at line 653.
- `src/plugins/registry.ts:87–92` — remove the `registerPlugin("bangs", { ... })` block. The WASM plugin's frontend is registered automatically via the plugin bundle's manifest (calculator does the same).

**Verify during implementation:** the native `BangsPlugin` used `network::website_metadata::WebsiteMetadataService` for favicon enrichment — the WASM port doesn't (favicons deferred). Confirm nothing else imports or constructs `metadata_service` specifically for bangs; `open_url` still uses it, so the construction stays.

After this step: `cargo build -p torchsnap-tauri && cargo test -p torchsnap-tauri` — the native tests related to bangs are gone along with the module; host test suite stays green. `bun run typecheck` (frontend) passes — `src/plugins/registry.ts` no longer references the removed file. Commit: `bangs: remove native plugin and derived bang.json`.
...
just check-wit
cd plugins && cargo check --workspace
just build-plugin bangs
just stage-bundled-plugins       # produces target/bundled-plugins/bangs.torchsnap
cargo build -p torchsnap-tauri
...
| `plugins/bangs/migrations/001_init.sql` | SQL schema (extracted from `schema.rs`) | 2 |
| `plugins/bangs/assets/bang.json` | 2.2 MB bundled DDG bang database | 2 |
| `plugins/bangs/src/lib.rs` | WASM guest port | 2 |
| `plugins/bangs/frontend/` | Scaffold (`package.json`, `vite.config.ts`, `tsconfig.json`, `env.d.ts`, `styles/settings.css`) | 2 |
| `plugins/bangs/frontend/src/BangsSettings.tsx` | Moved + SDK-adjusted from `src/plugins/bangs/BangsSettings.tsx` | 2 |
| `plugins/bundled.toml` | Add `"bangs"` | 2 |
| `just/bangs.just` | `dst="plugins/bangs/assets/bang.json"` | 2 |
...
- `todos/wasm/01kpk8jp3s737phqs239s9vt59-plugin-assets-wit-interface.md` — plugin-assets host interface (renamed to `assets`).
...
- `todos/01kn7v6ynyf580ax9jyyt25jgc-plugin-execute-data-param.md` — arbitrary-data parameter for `execute`. Required to remove the `PENDING_URL` thread-local hack (port-directly from the native `pending_url: Mutex<Option<String>>`). The `// HACK/FIXME` comment in `plugins/bangs/src/lib.rs` references this todo explicitly so the next author has a thread to pull.

---

## Known landmines captured in the plan

1. The bridge does NOT currently own its `PluginSource` — step 1a adds the field and rewrites `new()` to take `Arc<_>`.
2. `file_exists` vs. error classification — the host-side `assets::Host::read` runs `validate_plugin_path` BEFORE delegating to the trait, producing `InvalidPath` structurally and leaving `IoError` as the only failure category for the trait call.
...
8. No permission allowlist on `assets` — path validation is the security boundary. The WIT doc comment and `docs/api/plugin-development.md` both make this explicit.
...
- `/Users/jakob/Development/github/jakobwesthoff/torchsnap/plugins/bangs/src/lib.rs` (new)
