# Refactor: Gadget execute() data parameter

## Problem

Currently, gadgets that need to pass structured data from `search()` through to
...
The DuckDuckGo Bangs gadget is the first case where this becomes necessary:
`execute()` needs both the bang trigger and the original query to construct the
final URL.

## Proposed Solution

Add an optional opaque data parameter to the gadget entry/execute flow:

1. When a gadget creates a `ScoredEntry` in `search()`, it can attach an
   optional `data: Option<serde_json::Value>` (or `Option<Vec<u8>>`).
2. The `GadgetHost` does **not** serialize this data into the frontend-bound
...
4. When `execute(entry_id, action_id, app)` is called, the host looks up the
   stored data and passes it to the gadget as an additional parameter (or
...
- The `execute()` trait signature changes — breaking change for all gadgets.
- Need to decide: `Option<Value>` (flexible) vs typed per-gadget (complex).
