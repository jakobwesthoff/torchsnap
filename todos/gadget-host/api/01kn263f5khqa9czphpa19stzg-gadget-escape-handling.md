...
When a gadget's custom UI is activated via `PostAction::ShowCustomUI`
(either through a global shortcut or an execute action on a catalog
entry), ESC does not behave correctly:

- **Query non-empty + ESC**: should clear query and stay in gadget
  view. Actually exits the gadget view entirely.
- **Query empty + ESC**: should dismiss the launcher. Actually exits
  the gadget view but leaves the launcher open in an empty state.

## Activation paths

The clipboard gadget (catalog-mode, not query-mode) has two paths
that both result in `executeGadgetView` being set:

1. **Global shortcut** (`CmdOrCtrl+Shift+V`) → `handle_shortcut()`
   returns `PostAction::ShowCustomUI` → backend emits
   `activate-gadget-custom-ui` → frontend sets `executeGadgetView`
   and clears query.
2. **Execute action** (user selects "Clipboard History" catalog
   entry, presses Enter) → `execute()` returns
   `PostAction::ShowCustomUI` → frontend sets `executeGadgetView`
   and clears query.
...
1. Clears `executeGadgetView` → gadget view unmounts
2. Calls `setQuery("")` → no effect if already empty

When query is empty: `goBack()` exits the gadget view but never
...
When query is non-empty: `goBack()` exits the gadget view AND
...
`useSearch` took one extra render cycle to clear `customGadgetView`
...
Three ESC cases for execute-activated gadget views:

1. **Query non-empty** → clear query, stay in gadget view
...
3. **Query empty, activated via execute** → go back to launcher
   ...
   looking at search results and opened the gadget from there)

For prefix-activated views (not currently applicable to clipboard,
but relevant for future gadgets):
...
gadget was opened. Currently both activation paths call
...
- Gadget needs a "clear query, stay in view" action — `goBack()`
  always exits, no alternative exists today.
- Gadget needs to know activation mode (shortcut vs execute) to
  choose between dismiss and restore on exit. Both paths currently
  set `executeGadgetView` identically.
