...
- Letting a user patch or upgrade a bundled gadget independently of an
  app release, without having to rename the gadget and lose its data.

**Proposed UX:**
- Install refuses by default: "A system gadget with this ID already
  exists." (This is the v1 behaviour.)
- Settings panel exposes an "Advanced" toggle per-gadget:
  "Override system gadget". Ticking it lets the user retry the
  install (or keeps an already-installed overriding copy active).
- Overriding gadgets carry a persistent badge in the gadget list, and
  a one-click "Remove override" action removes the user copy and
  restores the system gadget.

**Security and UX notes:**
- An overriding gadget inherits the system gadget's SQL storage and
...
- When a bundled gadget version bumps in a later release, the override
  continues to shadow it. The gadget list should make that visible
...
- Should override be per-gadget only, or should a global "allow
