# Plugins UI — UX Polish

Three related issues in `src/settings/sections/PluginsManagementPanel.tsx`
...
**File:** `PluginsManagementPanel.tsx` → `PluginRowView`

The trailing slot in each `PluginRowView` row renders either a `<button>Uninstall</button>`
or a `<SourceBadge>`, whichever applies to the plugin's `sourceKind`. These
...
**File:** `PluginsManagementPanel.tsx` → `BannerView`, `PluginRowView`, `PluginsManagementPanel`
...
### 2b. Keep installed/uninstalled plugins in the list, marked "pending restart"

Currently the `rows` array in `PluginsManagementPanel` is derived purely from
`sourceKinds` (backend snapshot) crossed with `pluginMetadata` (frontend
registry). An uninstalled plugin disappears from the list immediately because
the backend no longer reports it. An installed plugin only appears after
...
which plugin IDs have been installed or uninstalled during this session. Rows
derived from that set should:

- Remain visible in the "Installed Plugins" list with a clear "Needs restart"
...
- Be present — but also visually deactivated — in the sidebar under the
  "Plugins" group in `SettingsSidebar`. This requires either propagating the
...
After `handleUninstall` completes successfully, the uninstalled plugin's row
must:
...
The cleanest approach is to add the plugin ID to the `pendingRestart` set
(from 2b) and pass an `isLockedOff` flag into `PluginRowView`, which then
...
- Hot-lifecycle removal (no restart needed at all) is tracked separately in
  `todos/wasm/…-plugin-hot-lifecycle.md` — do not conflate the two.
