...
2. **Sub-components need prop threading.** Extracting any helper from a
...
3. **Adding a new ambient capability is a breaking change.** Anything new
   forces every plugin to update its destructure list and the props
   interface bumps for everyone simultaneously.
4. **Symptom: the `pluginEnabled` prop addition.** Calculator's settings
...
carries an optional `launcher` field. Settings mounts pass `info +
runtime`; launcher view/inline mounts pass `info + runtime + launcher`.
