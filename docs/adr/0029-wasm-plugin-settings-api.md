...
1. `bindings::torchsnap::plugin::settings::Host` is implemented for
...
1. `WasmPluginBridge::enable()` stops ignoring `_ctx`. It clones
...
1. `WasmPluginBridge::setting_changed()` overrides the trait
...
1. The `settings` import is added to the linker automatically by
