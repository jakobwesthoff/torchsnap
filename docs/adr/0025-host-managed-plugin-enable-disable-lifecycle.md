...
1. Replace `is_enabled()`/`enabled_settings_key()` with a host-managed
   `AtomicBool` per plugin.
1. The host stores an `enabled.<plugin-id>` key at the top level of the
   settings store, not under `plugins.<id>.*`. Plugins cannot access or modify
   their own enabled state.
1. Add `setting_changed(key, value)` so plugins receive notifications about
   their own settings changes without needing `SettingsWatch` threads.
1. The host wraps each plugin in a `PluginSlot`:
...
6. `PluginContext` no longer carries a `notifier` field — only `settings` and
   `frecency`.
6. On first startup after upgrade, existing `plugins.<id>.enabled` values are
