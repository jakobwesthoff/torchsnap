...
1. **Cross-platform consistency.** We plan to ship on Windows (and
...
* **Frontend side.** A `TitleBar` component in `src/components/` owns
  ...
  * Platform-appropriate window controls (macOS: three traffic-light
    circles calling `getCurrentWebviewWindow().close() / .minimize() / .setFullscreen(!)`). Hover reveals the native-style glyphs via
  ...
  * `"titlebar"`: a classic macOS-style visible strip with
    ...
    lights at the native-style top-left corner (`left-[13px] top-[9px]`), and an optional centered `title` string rendered
