...
- `search_execute` (`mod.rs:47-62`) — wraps `GadgetHost::execute` in
...
2. The gadget's `search(stripped, Some(prefix))` runs on a single
   `tokio::task::spawn_blocking` (`gadget_host.rs:468-472`).
3. The returned `GadgetResponse` is fed to `process_gadget_response`
   with `allow_custom_ui = true` (`gadget_host.rs:479-481`).
4. Frecency bonuses are applied to the entries
   (`gadget_host.rs:498`), then a stable score-DESC sort is used —
   not the full `cmp_sort_key` — so the gadget's intended ordering
   for equal-score items is preserved (`gadget_host.rs:499`).
5. One `SearchResults` message is emitted with `matched_prefix:
   Some(prefix)`, followed by `Done` (`gadget_host.rs:502-510`).
...
`gadget_host.rs:519-545`, `search_catalogs_static` at
`gadget_host.rs:633-706`.
...
(`gadget_host.rs:536-545`). The frontend uses the catalog batch as the
canonical "new generation, recompute" signal.

#### Phase 2: query-gadget fan-out

`gadget_host.rs:547-609`.
...
`join_next` loop (`gadget_host.rs:565, 582-596`). The first arriving
...
- Rust catalog phase: `gadget_host.rs:703` (`cmp_sort_key`).
- Rust fan-out phase: `gadget_host.rs:580` (`cmp_sort_key`).
- Rust prefix phase uses score-DESC only, not the full key
  (`gadget_host.rs:499`); see the comment at
  `gadget_host.rs:492-497` for the reason.
...
`gadget_host.rs:498` (prefix), `:579` (fan-out), `:697-698` (catalog).
...
gadget (`gadget_host.rs:734`), so user intent is captured even when
the action errors.

The WIT `frecency` interface (`gadget-sdk/wit/torchsnap-gadget.wit:126-144`)
...
Native gadgets implement `crate::gadgets::Plugin`
(`gadgets/mod.rs:112-273`). The search-relevant entry points:
...
(`gadget-sdk/wit/torchsnap-gadget.wit:694-761`) and reach the host
through `WasmGadgetBridge` (`wasm/bridge.rs:903-950`), which adapts
the WIT types to the native `Plugin` trait. The bridge's
`search_prefixes()` reads `manifest.gadget.prefixes` from
...
- `gadgets/bangs/` — query gadget with a prefix (`!`). Demonstrates
  prefix-exclusive routing.
- `gadgets/calculator/` — query gadget with a prefix (`=`).
- `gadgets/emoji-picker/` — catalog gadget plus prefix browse mode
  with a `frecency::top-items` empty-query path.
- `gadgets/hello-world/` — minimal catalog gadget, no `search()`.
...
`GadgetViewRef` (`search/types.rs:251-256`) carries `gadgetId`,
