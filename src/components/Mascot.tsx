 ...
 *
 * Renders a `<picture>` element with WebP and PNG sources, automatically
 * serving 2x images for high-DPI displays. The component is purely
 * decorative — it carries no semantic meaning and uses `alt=""`.
 */
...
export interface MascotProps {
  ...
  size?: number;
  /**
   * CSS classes applied to the inner `<img>` element.
 ...
 *
 * Automatically serves 2x images for high-DPI displays via the `srcSet`
 * attribute on both WebP and PNG sources.
 *
...
export function Mascot({ variant = "original", size = 192, className }: MascotProps) {
  ...
  const size2x = size * 2;
  const webpSrcSet = `${basePath}-${size}.webp 1x, ${basePath}-${size2x}.webp 2x`;
  const pngSrcSet = `${basePath}-${size}.png 1x, ${basePath}-${size2x}.png 2x`;
  const src = `${basePath}-${size}.png`;
