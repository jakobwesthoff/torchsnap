...
impl bindings::torchsnap::plugin::command::Host for PluginState {
    ...
    > {
        use super::argv_matcher::matches as match_rule;
            ...
            None => {
                let Some(ctx) = self.path_context.as_ref() else {
                    return Err(WitErr::SpawnFailed(
                        "no path context available — command::run requires an enabled plugin".into(),
                    ));
                };
...
fn build_command_env(plugin_overrides: &[(String, String)]) -> Vec<(std::ffi::OsString, std::ffi::OsString)> {
...
> {
    ...
    match wait_result {
        Ok(Ok(status)) => {
            ...
            let (stdout_bytes, stdout_over, _) = stdout_join.unwrap_or_else(|e| {
                (Vec::new(), false, format!("stdout read error: {e}"))
            });
            let (stderr_bytes, stderr_over, _) = stderr_join.unwrap_or_else(|e| {
                (Vec::new(), false, format!("stderr read error: {e}"))
            });
...
async fn read_capped<R>(
    reader: R,
    cap: u64,
) -> std::io::Result<(Vec<u8>, bool, String)>
where
...
fn emit_command_audit(
    log_sender: &super::logging::channel::LogSender,
    plugin_id: &str,
    binary: &str,
    argv: &[String],
    outcome: &Result<
