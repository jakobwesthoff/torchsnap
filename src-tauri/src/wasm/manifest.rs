...
/// Scan `s` for `${name}` substitution tokens and reject
/// any whose `name` is not in
/// [`RECOGNIZED_PERMISSION_VARIABLES`]. Substitution
/// itself happens later (at bridge construction); this
/// pass exists so a typo in a variable name fails at
