...
// to `FilesystemCap::new`.
//
// At runtime, `read_file` / `file_exists` / `metadata`
// canonicalize the request path and match the real path
// against the GlobSet. A pattern allows a request only if
// both have been resolved through symlinks; this is what
// makes "follow symlinks" safe, since a symlink inside an
// allowed location pointing outside the allow set is rejected
// after canonicalization.
//
// `std::fs::read` / `std::fs::canonicalize` block the OS
// thread, so the public methods wrap the I/O in
...
/// Filesystem capability error. Mirrors the WIT `fs-error`
...
/// Metadata for a single file, returned by
/// `FilesystemCap::metadata`.
#[derive(Debug)]
...
impl FilesystemCap {
    ...
    /// `GlobSet`.
    pub fn new(patterns: &[String], resolver: &impl PathResolver) -> anyhow::Result<Self> {
        let allowlist = compile_fs_patterns(patterns, resolver)?;
        Ok(Self { allowlist })
    }

    /// Read the contents of `path` if it passes validation
    /// and the allowlist gate.
    ...
    /// Check whether `path` exists and is permitted by the
    /// allowlist. Returns `false` for paths that fail
    /// validation, are not found, or are outside the
    /// allowlist.
    pub fn file_exists(&self, path: &str) -> bool {
        tokio::task::block_in_place(|| resolve_request(&self.allowlist, path).is_ok())
    }

    /// Return metadata for `path` if it passes validation
    /// and the allowlist gate.
...
/// `FsAllowlist`. Called by `FilesystemCap::new`.
///
/// For each pattern:
...
fn validate_request_path(path: &str) -> Result<(), FilesystemError> {
    if path.is_empty() {
        return Err(FilesystemError::InvalidPath("empty".into()));
    }
    if path.contains('\0') {
        return Err(FilesystemError::InvalidPath("contains NUL".into()));
    }
...
fn resolve_request(allowlist: &FsAllowlist, path: &str) -> Result<PathBuf, FilesystemError> {
...
mod tests {
    ...
    fn resolve_request_denies_path_outside_allowlist() {
        ...
        match resolve_request(&allowlist, outside.to_str().unwrap()) {
    ...
    fn resolve_request_returns_not_found_for_missing_path() {
        ...
        match resolve_request(&allowlist, missing.to_str().unwrap()) {
    ...
    fn resolve_request_follows_symlink_target_through_allowlist() {
        ...
        resolve_request(&allowlist, link.to_str().unwrap()).expect("symlink-to-allowed");
    }
    ...
    fn resolve_request_denies_symlink_pointing_outside_allowlist() {
        ...
        match resolve_request(&allowlist, link.to_str().unwrap()) {
            Err(FilesystemError::PermissionDenied(_)) => {}
    ...
    fn resolve_request_rejects_traversal_in_request() {
        ...
        match resolve_request(&allowlist, "/etc/../etc/hosts") {
            Err(FilesystemError::InvalidPath(_)) => {}
            other => panic!("expected InvalidPath, got {other:?}"),
        }
    }

    // ─── FilesystemCap construction ────────────────────────

    #[test]
    fn new_compiles_patterns() {
        let tmp = TempDir::new().expect("tempdir");
        let ctx = ctx_for(&tmp);
        std::fs::create_dir_all(&ctx.platform.xdg_config).expect("mkdir xdg-config");

        let cap = FilesystemCap::new(&["${xdg-config}/*.txt".into()], &ctx)
            .expect("construction succeeds");
        assert!(!cap.allowlist.canonical_patterns.is_empty());
    }
}
