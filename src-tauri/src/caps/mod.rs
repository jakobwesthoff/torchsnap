...
impl ProvisionedCaps {
    ...
    // Capability accessor surface generated by cap_accessor!, same as the
    // methods above; not every provisioned capability has an in-repo
    // accessor caller.
    cap_accessor!(#[allow(dead_code)] filesystem -> FilesystemCap);
    cap_accessor!(#[allow(dead_code)] command -> CommandCap);
    cap_accessor!(#[allow(dead_code)] sql_storage -> SqlStorageCap);
    cap_accessor!(#[allow(dead_code)] website_metadata -> WebsiteMetadataCap);
    cap_accessor!(#[allow(dead_code)] frecency -> FrecencyCap);
}
