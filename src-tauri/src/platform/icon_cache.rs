...
// Icon Cache
//
// Manages a disk-based cache of application icons extracted
// by the platform `IconExtractor`. Icons are stored as PNG
// files with blake3-hashed filenames derived from the app
// path and bundle ID.
//
...
impl IconCache {
    ...
    pub fn ensure_icon(&self, app: &DiscoveredApp) -> Option<String> {
        ...
        let png_path = self.cache_path(&key);
        ...
        if self.is_cache_valid(&png_path, &app.path) {
            return Some(png_path.to_string_lossy().into_owned());
        }

        // Cache miss or stale — extract the icon from the app bundle.
        match self.extractor.extract(&app.path) {
            Ok(Some(png_bytes)) => {
                ...
                if let Err(e) = fs::write(&png_path, &png_bytes) {
                    eprintln!("write icon cache {}: {e:#}", png_path.display());
                    return None;
                }
    ...
    /// cache keys. Any `.png` file in the cache dir whose stem is
